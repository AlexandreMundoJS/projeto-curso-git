...
public class StorageUploadActivity extends AppCompatActivity implements View.OnClickListener {
    ...
    private void button_Upload(){
        if (Util.statusInternet(getBaseContext())){
            if(imageView.getDrawable() != null){
                ...
                Toast.makeText(getBaseContext(), "Não existe imagem para realizar o upload.", Toast.LENGTH_LONG).show();
            }
        }else{
            Toast.makeText(getBaseContext(), "Erro de conexão. Verifique seu Wifi ou 3G.", Toast.LENGTH_LONG).show();
        }
                ...
                uploadTask.continueWithTask(new Continuation<UploadTask.TaskSnapshot, Task<Uri>>(){
                    ...
                    public void onComplete(@NonNull Task<Uri> task) {
                        if(task.isSuccessful()){
                            ...
                            alerta.show(getFragmentManager(), "3");
    ...
    private void item_camera(){
        if (ContextCompat.checkSelfPermission(getBaseContext(), Manifest.permission.CAMERA) == PackageManager.PERMISSION_DENIED){
            DialogAlerta alerta = new DialogAlerta("Permissão Necessária", "Acesse as configurações do aplicativo para poder utilizar a câmera no nosso aplicativo");
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent data){
        ...
        if (resultCode == RESULT_OK){
            if (requestCode == 0){
                ...
                if (uri_imagem != null){ //Verificar resposta da câmera
                    Glide.with(getBaseContext()).asBitmap().load(uri_imagem).listener(new RequestListener<Bitmap>() {
                        @Override
                        public boolean onLoadFailed(@Nullable GlideException e, Object model, Target<Bitmap> target, boolean isFirstResource) {
                            Toast.makeText(getBaseContext(), "Erro ao carregar imagem", Toast.LENGTH_LONG).show();
